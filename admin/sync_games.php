...
foreach ($files as $filepath) {
    ...
    $provider = (count($parts) > 1) ? $parts[0] : 'general';
    ...
    if ($res && mysqli_num_rows($res) == 0) {
        $insert_sql = "INSERT INTO demo_games (demo_name, demo_gamelink, demo_provider) VALUES ('" . mysqli_real_escape_string($data, $filename) . "', '#', '" . mysqli_real_escape_string($data, $provider) . "')";
